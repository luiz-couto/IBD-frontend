...
const Innovation = () => {
    ...
    useEffect(() => {
        query(`SELECT Indicator FROM innovation GROUP BY indicator`).then((data) => {
            setIndicators(data.rows);
            setSelectedIndicator(data.rows[0].indicator)
        });
        query(`SELECT Country FROM innovation GROUP BY Country`).then((data) => {
    ...
    useEffect(() => {
        ...
        setLoadingData(true);
        query(`SELECT country, time, value FROM innovation WHERE Indicator='${selectedIndicator}'`).then((data) => {
    ...
    return (
        <div>
                <img src={require('../../pages/Innovation/background.jpg')} style={{ width: '100vw', position:'absolute',
                height: 'auto', maxHeight: '100vh', backgroundSize: 'cover' }} />
                <Layout>
                    <SiderMenu color={'rgb(0,0,0)'} selectedItem={["4"]}/>
                    <Layout>
                        <Content>
                        <Col>
